...
export default function BoutiqueClient({ initialProducts, initialCategories }) {
  const { locale, t, isAr } = useI18n();
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("");
  const [sortBy, setSortBy] = useState("name");

  const searchLocal = (query, products) => {
  ...
  const filtered = useMemo(() => {
    let result = searchQuery ? searchLocal(searchQuery, initialProducts) : [...initialProducts];
      ...
      <div className="container">
        ...
        {filtered.length > 0 ? (
          ...
          <div className={styles.emptyState}>
            ...
            >
              {t.boutique?.resetBtn || "Réinitialiser"}
            </button>
          </div>
        )}
      </div>
